...
import { ArrowLeft, Pencil, Tent } from "lucide-react";
...
function toFormValues(p: ParticipantSummary): FormValues {
  return {
    isMemberTibl: p.isMemberTibl ? "true" : "false",
    church: p.isMemberTibl ? "" : p.church,
    occupationStatus: p.occupationStatus,
    firstTime: p.firstTime ? "true" : "false",
    baptized: p.baptized ? "true" : "false",
    fullName: p.fullName,
    gender: p.gender,
    birthDate: p.birthDate.slice(0, 10),
    phone: formatAngolaPhone(p.phone),
    whatsapp: formatAngolaPhone(p.whatsapp),
    email: p.email,
    allergicTo: p.allergicTo ?? "",
    maritalStatus: p.maritalStatus ?? "SINGLE",
    bringingChildren: p.bringingChildren ? "true" : "false",
    numberOfChildren: p.numberOfChildren ? String(p.numberOfChildren) : "",
    transportRequired: p.transportRequired ? "true" : "false",
    transportStopId: p.transportStop?.id ?? "",
    ownTransportType: p.ownTransportType ?? ("" as FormValues["ownTransportType"]),
    carSeats: p.carSeats != null ? String(p.carSeats) : "",
    carRouteStops: p.carRouteStops ?? "",
    tentRequired: p.tentRequired,
    mattressRequired: p.mattressRequired,
    tentsCanProvide: String(p.tentsCanProvide ?? 0),
    mattressesCanProvide: String(p.mattressesCanProvide ?? 0),
    wantsToBuyTent: p.wantsToBuyTent ? "true" : "false",
    tentPurchaseTypeId: p.tentPurchaseType?.id ?? "",
    tentPurchaseQuantity: String(p.tentPurchaseQuantity || 1),
    wantsToBuyMattress: p.wantsToBuyMattress ? "true" : "false",
    mattressPurchaseQuantity: String(p.mattressPurchaseQuantity || 1),
    isSponsored: p.isSponsored ? "true" : "false",
    paymentStatus: p.paymentStatus,
    paidInHand: p.paidInHand === false ? "false" : "true",
    paymentAmountPaid: String(p.paymentAmount ?? ""),
  };
}
...
export default function EditParticipantPage() {
  ...
  useEffect(() => {
    if (!session) return;
    apiFetch<ParticipantSummary>(`/participants/${params.id}`, { token: session.accessToken })
      .then((p) => {
        setParticipant(p);
  ...
  useEffect(() => {
    ...
    else if (isMemberTibl === "false" && participant.isMemberTibl) setValue("church", "");
    // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  async function onSubmit(values: FormValues) {
    ...
    if (!sponsored) {
      ...
      // Only demand a new proof when there isn't one on file already — the
      // admin may be correcting an unrelated field, not the payment itself.
      if (values.paidInHand === "false" && !paymentProof && !participant.paymentProofPath) {
    ...
    try {
      ...
      const updated = await apiFetch<ParticipantSummary>(`/participants/${participant.id}`, {
        method: "PATCH",
        token: session.accessToken,
        body: formData,
      });

      toast.success(`Inscrição de ${updated.fullName} atualizada.`);
      router.push("/admin/participantes");
    } catch (error) {
      toast.error(error instanceof ApiError ? error.message : "Não foi possível guardar as alterações.");
    } finally {
      setSubmitting(false);
    }
  }
  ...
  if (loadError) {
    return (
      <div className="animate-in fade-in mx-auto max-w-lg space-y-4 py-16 text-center duration-500">
        <p className="text-sm text-muted-foreground">Não foi possível carregar esta inscrição.</p>
        <Link href="/admin/participantes" className="text-sm text-primary underline underline-offset-2">
  ...
  if (!participant) {
    return <PageLoading label="A carregar inscrição..." />;
  }
    ...
    <div className="animate-in fade-in mx-auto max-w-2xl space-y-6 duration-500">
      ...
      <div className="text-center">
        ...
        <p className="text-sm text-muted-foreground">
          {participant.fullName} · {participant.registrationNumber}. Todas as alterações ficam registadas no
                ...
                {!sponsored && (
                  <div className="animate-in fade-in slide-in-from-top-1 space-y-5 duration-300">
                    ...
                    <div className="space-y-2">
                      ...
                      {participant.paymentProofPath && !paymentProof && (
                        <p className="text-xs text-muted-foreground">
                          Já existe um comprovativo carregado. Só anexe um novo ficheiro se quiser substituí-lo.
                        </p>
                      )}
                      <FileUpload
                        value={paymentProof}
