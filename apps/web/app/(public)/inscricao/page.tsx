...
import type {
  ...
  TentTypeSummary,
  TransportStopSummary,
} from "@dunamis/types";

export const dynamic = "force-dynamic";

export default async function InscricaoPage() {
  const [stops, tentTypes, status, settings] = await Promise.all([
      ...
      <div className="mx-auto w-full max-w-2xl px-6 py-16">
        ...
        {status.open ? (
          <RegistrationForm
            stops={stops}
            tentTypes={tentTypes}
            pricing={{ student: settings.paymentAmountStudent, worker: settings.paymentAmountWorker }}
          />
        ) : (
